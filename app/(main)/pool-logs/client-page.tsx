...
const Post = ({ data }: { data: LogMeta & { slug: string } }) => (
  <div className="w-full flex flex-row items-center py-5 px-2.5 odd:rounded-lg odd:bg-[#ebebeb]">
    <div className="md:w-1/2">
      <Link href={`/pool-logs/${data.slug}`} className="text-xl">{data.title}</Link>
...
const ClientPage = ({ posts }: { posts: (LogMeta & { slug: string })[] }) => {
  ...
  if (query && query.trim().length > 0) {
    ...
    results = posts.filter((post) => post.title.toLowerCase().includes(lcQuery));
  }
