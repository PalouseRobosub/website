...
Note that $M$ must be an invertible matrix. If $M$ has fewer than 6 thrusters, the solution is not solvable because the submarine does not have control of 6 axes (it has less than 6 motors, so it can therefore not have control of 6 axes). If $M$ has greater than 6 thrusters, then the system is over-actuated and a [SVD pseudo-inverse](https://en.wikipedia.org/wiki/Singular_value_decomposition) calculation can be done to determine $M^\{-1\}$.
