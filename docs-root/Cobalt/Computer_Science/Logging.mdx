# Logging
We use the ROS logging API for having programs produce output. See [The official documentation for more details](http://wiki.ros.org/roscpp/Overview/Logging).
...
Here are some [ general guidelines](http://wiki.ros.org/Verbosity%20Levels) for when to use each level. If you use the FATAL message, your node should use a call to exit(nonzero value) very soon (if not immediately) after publishing the message.
...
## Programatically
Yes, you can make a node set its log level by itself! Check out this [ example](http://wiki.ros.org/roscpp/Overview/Logging#Setting_Verbosity_Levels ).
